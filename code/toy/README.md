...
命名约定：`code/toy/lN_*/`（脚本）与 `data/toy_results/lN_*/`（结果）**一一对应**。
理论文档在 `docs/notes/theory/`，图在 `docs/figs/theory/`。
...
| | |
|---|---|
| 脚本 | `code/toy/l1_lookup_replay/toy6_lookup_replay.py`（纯 torch） |
| 结果 | `data/toy_results/l1_lookup_replay/main_matrix_20260805/`（160 runs, 4.5 MB）<br>`markov_5ep/`（Markov 数据上的 5 epoch 变体）<br>`phase1_confirmation_20260821/`（独立复现证据） |
...
| 输入 | `data/toy_results/l5_optimizer_artifact/inputs/exact_frequency_distribution.json`（真实频率直方图 fixture） |
| 结果 | `data/toy_results/l5_optimizer_artifact/rmsprop_v_sawtooth/`（5 臂 JSON + `_arms.{png,svg}`） |
...
```bash
python code/toy/l2_markov_exact/markov_clean_unigram.py     # ~35 s，纯 numpy
python code/toy/l3_sampling_law/gap_vs_samples_unigram.py   # 纯 numpy
python code/toy/l5_optimizer_artifact/mc_table_size.py      # 纯 numpy
python code/toy/l5_optimizer_artifact/rmsprop_v_sawtooth.py # torch，支持 MPS
python code/toy/l1_lookup_replay/toy6_lookup_replay.py      # torch，160 runs 较慢
```
