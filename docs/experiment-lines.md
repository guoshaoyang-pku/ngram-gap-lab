...
| **M9** | backbone LR × 固定绝对 table LR | 去掉旧扫描中 table LR 随 `--lr` 共变的混淆；检验动力学时间尺度与平衡位置 | `blrabs_{input,nogram}_*` | `queue_v5_backbone_lr_abslock.sh` | 预登记：完成后新增 | 预登记：完成后新增 | §42 | 🔄 running；14 run，table LR 恒为 0.0768，val/freq=50 |
