...
| **M3b** | 表学习率 × β₂ 消融深挖（附录） | β₂ 与表学习率的消融及交互；**高表学习率体检（发现 ×2/×4 崩坏）** | `nglab*_opt_*_fixed` + 补点 `nglab*_b2_099_lr1` | 手工启动（补点脚本见任务目录） | `tasks/apendix_lr_beta_ablation/` | `tasks/apendix_lr_beta_ablation/figs/` | §9 系列 | 🟡 进行中（2 个补点跑中） |
