...
FIGS_DIR = os.environ.get("NGRAM_GAP_V10_FIGS_DIR",
                          os.path.join(REPO_ROOT, "docs", "figs", "main"))
...
def compute_epoch_boundaries(data):
    """Return [(step, 'epoch N'), ...] from the input run's train_log epoch field."""
    pts = data.get("input", {}).get("train_log", [])
    boundaries = []
    prev_epoch = None
    for p in pts:
        epoch = p.get("epoch")
        if epoch is not None and epoch != prev_epoch:
            if prev_epoch is not None:
                boundaries.append((int(p["step"]), f"epoch {int(epoch)}"))
            prev_epoch = epoch
    return boundaries
...
def gen_fig_gap_loss(data):
    ...
    """
    html = HTML_WRAP.format(title="注入点消融：Gap + Loss", note="v/y/input 三注入点对比，vanilla nanoGPT + bigram+trigram + seed42 + 2000步，validation 每 10 步",
...
def gen_fig_loss_norm(data):
    ...
    epoch_shapes = [
        {"type": "line", "x0": step, "x1": step, "y0": 0, "y1": 1,
         "yref": "paper", "line": {"color": "#ccc", "dash": "dot"}}
        for step, _ in compute_epoch_boundaries(data)
    ]
    ...
    """
    html = HTML_WRAP.format(title="Table Norm × Loss 对齐 (input 注入)", note="table RMS 增长与 gap 出现的时间对齐关系（validation 每 10 步）",
                            body=body, plotly=PLOTLY_HEAD, script=script)
    with open(out, "w") as f:
        f.write(html)
    print(f"[fig] {out}")
...
def gen_fig_gap_by_freq(data):
    ...
    plot();
    """
    html = HTML_WRAP.format(title="频率 bin 分解：train / val / gap", note="input 注入 run。每条线 = 一个频率桶。novel = train 中未出现的 context（仅 val）。虚线 = epoch 边界（数据自动标注）。validation 每 10 步。",
                            body=body, plotly=PLOTLY_HEAD, script=script)
    with open(out, "w") as f:
        f.write(html)
    print(f"[fig] {out}")
...
def gen_fig_gap_loglog(data):
    """Figure 6: log-log view (frequency vs gap) for power-law checks."""
    out = os.path.join(FIGS_DIR, "fig_gap_vs_frequency_loglog.html")
    d = data.get("input", {})
    fb_pts = d.get("freq_bin", [])
    if not fb_pts:
        print("[fig] fig_gap_vs_frequency_loglog: no freq_bin data, skipping")
        return
    last = fb_pts[-1]
    bounds = {
        "novel": (0.5, 0.5), "1": (1, 1), "2": (2, 2), "3": (3, 3),
    ...
    <div id="loglog_chart" class="chart"></div>
    """
    script = f"""
    var logData = {json.dumps(log_data)};
    var colors = {{bigram: "#353d79", trigram: "#c4493d"}};
    function makeTrace(branch) {{
      ...
      return {{
        x: d.map(function(x){{return x.x}}),
        y: d.map(function(x){{return Math.max(x.gap, 1e-4)}}),
        customdata: d.map(function(x){{return [x.bucket, x.x_low, x.x_high, x.train_frac, x.val_frac, x.train_loss, x.val_loss]}}),
        mode: "lines+markers",
        name: branch,
        line: {{color: colors[branch], width: 2.5}},
    ...
    function setVisible(which, button) {{
      ...
      var visibility = which === "both" ? [true, true] :
        (which === "bigram" ? [true, "legendonly"] : ["legendonly", true]);
      Plotly.restyle("loglog_chart", {{visible: visibility}});
    }}
    Plotly.newPlot("loglog_chart", [makeTrace("bigram"), makeTrace("trigram")], {{
      title: "Log–Log: final gap vs training hit count",
      xaxis: {{title: "training hit count (log)", type: "log", dtick: 1}},
      yaxis: {{title: "final gap (log)", type: "log", dtick: 1}},
      margin: {{l:70,r:30,t:50,b:65}}, showlegend: true,
      legend: {{x: 0.02, y: 0.98}}
    }});
    """
    html = HTML_WRAP.format(
        title="Log–Log：frequency 与 gap",
        note="双对数视图：用于检查 frequency–gap 是否近似幂律关系（若为直线则接近幂律）。",
        body=body, plotly=PLOTLY_HEAD, script=script)
    with open(out, "w") as f:
        f.write(html)
    print(f"[fig] {out}")
