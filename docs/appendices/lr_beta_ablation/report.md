...
> **状态**：🟡 进行中 —— 两个补点（β₂=0.99 · LR×1）在集群运行（2026-08-24 10:36 启动，预计 ~50 min）

> **★ 决策（2026-08-24 用户拍板）**：新标准 = **β₂=0.99（无动量）· 表学习率 ×2（0.008）**。
> 依据：β₂=0.99 · LR×2 分叉 @460 步、1000 步时已有 2.3 gap、val 健康；终点与 ×4 相同但更稳
> （无 10.8 尖峰、表 norm 1.32 vs 2.54）。LR×1 的两个补点让它慢慢跑完作对照，不再影响决策。
> `code/train.py` 默认值已同步：`table_betas=(0.0,0.99)`、`table_lr_scale=2.0`。
...
1. **当前标准使用表学习率 ×2**（0.008）；历史 ×1/×4 结果仅用于消融和健康性判断；
