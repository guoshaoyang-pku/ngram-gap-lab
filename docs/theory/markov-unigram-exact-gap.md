...
Code: `toy/markov_clean_unigram.py` (self-contained, pure numpy, ~35 s)
Outputs: `toy/results/markov_clean/{fig1_unigram_5epoch,fig2_unigram_saturation,fig3_bigram}.{png,svg}`, `results.json`
...
- `toy/results/markov_bc_5epoch_fast` (M=512, 5 ep, per-epoch full-batch
...
- `toy/results/markov_5epoch_dense` / `markov_5epoch_sgd` (M=64, N=2^20):
...
- `toy/results/toy6_markov_5ep`: gap -1.3e-5 +- 7.5e-5, table_rms = 0 --
