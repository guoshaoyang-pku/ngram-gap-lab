...
未知平方残差用可观测的 `M(f)` 代理。核心 bigram 区间 `f≤721` 中，同一几何桶、同一
val-token 权重下，实测斜率 −0.2548、`M(f)` 斜率 −0.2579；单幅度加权 `R²=.9638`。
只在 `f≤100` 拟合幅度并外推 `100<f≤721`，`R²=.8837`。trigram 实测/计数斜率
−0.3181/−0.3194，但两者窗口与权重不同，只作 cross-check；复算见
`docs/figs/theory/theory_good_turing_exponent_summary.csv`。
