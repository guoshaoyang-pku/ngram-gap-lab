...
### T2 · 重判 β₂ 结论（最高优先，直接影响 setting 决策）

**问题**：`experiment-log.md` §9c/§9d 的结论「β₂ 影响 ≤ ±0.4，远小于 LR 效应」**建立在 B2 bug 之上**——
那批 run 的 `--table_betas` 根本没生效，全部实际跑的 b2=0.999。所以**目前没有任何有效的 β₂ 证据**。

修正后已知：`nglab2x_opt_rmsprop_2x_b2_099` 从 0.643 → **1.995（+210%）**。

**做什么**
1. 清点 `runs_fixed/` 里所有 `*b2_*_fixed` run，确认哪些是**修复后真正跑出 β₂ 差异**的。
2. 若 `_fixed` 数据已覆盖 β₂ ∈ {0.98, 0.99, 0.999, 0.9999} × {1x, 2x}，直接重写结论。
3. 若覆盖不全，列出缺口并按 T2b 补跑。
4. 明确回答用户的问题：**β₂ = 0.99 是否比 0.999 更合理**。给出证据，不要凭直觉。

**T2b（需 GPU）**：补跑缺失的 β₂ 点。
setting = `agents.md` §1，只改 `--table_betas 0.0,<b2>`。1x + 2x 各一组，seed 42。
命名 `nglab{1,2}x_opt_rmsprop_b2_<b2>_v2`（v2 表示 post-fix 波次）。

**验收**：§9c/§9d 重写完成，结论有 `_fixed` 数据支撑；β₂ 默认值给出明确建议。
...
`ngram5_freq_gap/cluster/run_big_continuous.sh` 等 3 个脚本是未完成线
（closure 状态 `DATA_GEN_RUNNING`，`meta.json` 从未产出，输入 `data_split.full163.json` 不在仓库）。
已加 `[INCOMPLETE]` banner。

**需用户决定**：复活（补 data split 重跑）还是退役（删除 3 个脚本）。
按「bug 内容彻底删除」原则倾向退役，但这是未完成而非 bug，故先问。

---

## P2 · 下一阶段实验（需 GPU，按序做）

### T8 · 长时程 no-ngram 对照【必须先于 T9】

**科学问题**：**backbone 单独长训会不会也产生 gap？**
如果会，那 gap 就不能全部归因给 n-gram 表，后续所有实验的解释口径都要改。

**为什么必须先做**：现有 no-ngram 对照只到 2000 步（`nglab1x_v10_nogram_fixed`，gap 0.245）。
而 toy L1 的证据显示 backbone 长训到 104 epoch 后低频键 gap 会从 13.2 涨到 20.7——
**backbone 确实会过拟合，只是慢且浅**。不先确认这条基线，T9 缩小数据量放大 gap 之后无法归因。
...
- 第二批（依赖第一批）：T2、T3
- 第三批（GPU）：T8 → 然后 T9、T10 并行；T2b、T6 视 T2 结论插入
