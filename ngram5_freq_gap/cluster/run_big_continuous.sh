...
#
# !! STATUS: INCOMPLETE / NOT RUNNABLE AS-IS !!
#   The full-163 line never finished (closure status: DATA_GEN_RUNNING, meta.json
#   never produced).  Its input `data_split.full163.json` is NOT in this repo, so
#   the scp below will fail.  Provide that file (or regenerate the split) before
#   using this script.  See docs/notes/plans/plan-3-fix-and-backfill.md T7 for the decision on whether to revive
#   or retire this line.
set -euo pipefail
...
scp "$REPO_ROOT/data_split.full163.json" "$SSH_HOST:$CLUSTER_ROOT/data_split.json"
