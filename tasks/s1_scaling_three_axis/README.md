...
> 完整计划：`docs/notes/plans/plan-3-fix-and-backfill.md`（plan.md 存档）。
...
## Pilot 结果（seed 42，已跑通，2026-08-24）
...
**结论（pilot）**：gap 由 n-gram 表重播产生而非 backbone；重播轮数越多 gap 越大。
...
**结论（pilot）**：table 越小 gap 越小，单调递减 → 支持 collision pooling 假设
（小 table 的 hash 碰撞稀释逐 context 记忆），需 full grid 确认形状。
...
- [x] Pilot（7 run，seed 42）：epoch 4 + table 3，全部完成
