...
LOG="${SYNTH_LOG:-$TASK_ROOT/results/synth_pipeline.log}"
...
DATA_B="${SYNTH_DATA_B:-$TASK_ROOT/results/inputs/synth_B_lowrank_sparse}"
{
  ...
  if [ ! -f "$DATA_A/meta.json" ]; then
    ...
    "$PY" "$TASK_ROOT/synthetic_transition_gen.py" --out-dir "$DATA_A" --scheme sparse_restart || { echo "GEN_A FAILED"; exit 1; }
  else
    echo "--- gen A: exists, skip ---"
  fi
  if [ ! -f "$DATA_B/meta.json" ]; then
    echo "--- gen B (lowrank_sparse) ---"
    "$PY" "$TASK_ROOT/synthetic_transition_gen.py" --out-dir "$DATA_B" --scheme lowrank_sparse || { echo "GEN_B FAILED"; exit 1; }
  else
    echo "--- gen B: exists, skip ---"
  fi
  if [ ! -f "$DATA_A/meta.json" ] || [ ! -f "$DATA_B/meta.json" ]; then
    echo "--- prep A ---"
    "$PY" "$TASK_ROOT/synthetic_prep.py" --data-dir "$DATA_A" || { echo "PREP_A FAILED"; exit 1; }
    echo "--- prep B ---"
    "$PY" "$TASK_ROOT/synthetic_prep.py" --data-dir "$DATA_B" || { echo "PREP_B FAILED"; exit 1; }
  else
    echo "--- prep: meta.json present, skip ---"
  fi
  echo "--- smoke (GPU ${SYNTH_GPU_A:-3}) ---"
  bash "$TASK_ROOT/cluster/run_synth_3602.sh" smoke || { echo "SMOKE FAILED"; exit 1; }
  echo "--- all (2 waves, 2 GPUs) ---"
  bash "$TASK_ROOT/cluster/run_synth_3602.sh" all || { echo "ALL FAILED"; exit 1; }
