...
| LR schedule | `--lr_schedule warmup_constant --warmup_steps 100`；step 1–100 从 0.1×LR 线性 warmup，之后固定 LR；所有新实验禁止 warmdown |
...
| lr schedule | **warmup_constant（warmup 后固定 LR）** | `warmup_steps=100`：step 1–100 从 0.1×LR 线性升至 1×LR，之后不衰减，不随 epoch / 总 steps 改变。所有新实验必须显式传此 schedule；warmdown 仅限重跑已有历史 run。零 warmup 的 `constant` 仅作优化器诊断，未通过收敛质检不得用于 gap 结论 |
