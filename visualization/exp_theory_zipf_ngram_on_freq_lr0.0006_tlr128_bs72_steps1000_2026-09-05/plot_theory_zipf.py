...
def plot_gap_table_rms(
    ...
    """Plot global gap and recorded bigram/trigram table RMS on twin axes."""
    ...
    bi_line = ax_rms.plot(steps, bigram_rms, color="#1f77b4", linewidth=1.6, label="Bigram table RMS")
    tri_line = ax_rms.plot(steps, trigram_rms, color="#ff7f0e", linewidth=1.6, label="Trigram table RMS")
    ax_gap.axhline(0.0, color="black", linewidth=0.8)
    ax_gap.set_xlabel("Logged step")
    ax_gap.set_ylabel("Val − train", color="#2ca02c")
    ax_rms.set_ylabel("Table RMS", color="#555555")
    ax_gap.set_title(f"Gap and n-gram table RMS ({summary['run_id']})")
    ax_gap.grid(alpha=0.25)
    lines = gap_line + bi_line + tri_line
    ax_gap.legend(lines, [line.get_label() for line in lines], frameon=False, loc="upper left")
    fig.text(
        0.01,
        0.01,
        f"Recorded table norms; seed {summary['seed']}, endpoint step {summary['steps']}",
        fontsize=9,
    )
    fig.tight_layout(rect=(0, 0.04, 1, 1))
